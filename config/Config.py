...
class Config:
    ...
    def getConfig(self, serverID: int, configStr: str, default: Any = "") -> Any:
        if not self.__config:
            self._loadConfig()
        return self.__config.get(str(serverID), {}).get(configStr, default)

    def getFormatConfig(self, serverID: int, configStr: str, template: str) -> str:
        ret = ""
        val = self.getConfig(serverID, configStr)
        if val:
            data = {configStr: val}
